...
function AdminDashboard() {
  ...
  function exportCSV(courseId: string, courseTitle: string) {
    ...
    const header = "الاسم,الجوال,البريد,اسم الجمعية,رقم الترخيص,الحالة,تاريخ التسجيل";
    const rows = regs.map(
      (r) =>
        `"${r.full_name}","${r.phone}","${r.email || ""}","${r.association_name || ""}","${r.license_number || ""}","${r.status}","${new Date(r.registered_at).toLocaleDateString("ar-SA")}"`
    );
                ...
                >
                  <div className="p-5 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      >
                        {copiedId === course.id ? <Check size={18} /> : <Link2 size={18} />}
                      </button>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="text-xs text-gray-500 border-b border-gray-100">
                                ...
                                <th className="text-right px-4 py-2 font-medium">الجمعية</th>
...
function AddCourseForm({ onSuccess }: { onSuccess: (c: Course) => void }) {
  ...
  const checkboxLabelClass =
    "flex items-center gap-2 cursor-pointer bg-gray-50 p-3 rounded-xl border border-gray-200 hover:border-brand-orange transition-colors";
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {/* الحقول المطلوبة من المسجلين */}
          <div className="md:col-span-2 space-y-2">
            <p className={labelClass}>الحقول المطلوبة من المسجلين:</p>
            <label className={checkboxLabelClass}>
              ...
              />
              <span className="text-sm text-brand-dark font-medium">البريد الإلكتروني</span>
            </label>
            <label className={checkboxLabelClass}>
              <input
                type="checkbox"
                name="require_association_name"
                className="w-4 h-4 accent-brand-orange"
              />
              <span className="text-sm text-brand-dark font-medium">اسم الجمعية</span>
            </label>
            <label className={checkboxLabelClass}>
              <input
                type="checkbox"
                name="require_license_number"
                className="w-4 h-4 accent-brand-orange"
              />
              <span className="text-sm text-brand-dark font-medium">رقم الترخيص</span>
            </label>
          </div>
